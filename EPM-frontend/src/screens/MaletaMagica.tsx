...
export default function MaletaMagica() {
  ...
  const handleDragEnd = (event: any, info: any, item: any) => {
    // Check if the item was dropped in the lower half of the screen (where the suitcase is)
    // For a simple implementation, if y offset is positive enough
    if (info.offset.y > 100) {
      setInSuitcase([...inSuitcase, item.id]);
  ...
  return (
    <div className="min-h-screen w-full flex flex-col p-6 bg-surface relative overflow-hidden">
      ...
      <main className="flex-1 flex flex-col items-center justify-between pb-10 z-10 relative w-full max-w-4xl mx-auto">
           ...
           <AnimatePresence>
             {items.map((item) => (
               ...
               >
                 ...
                 <span className="text-sm font-medium bg-surface-container-high px-3 py-1 rounded-full pointer-events-none">{item.label}</span>
               </motion.div>
             ))}
           </AnimatePresence>
        ...
        {/* The Suitcase */}
        <div className="relative flex justify-center w-full max-w-sm mt-auto mb-10">
           {isSparkling && (
           ...
           <div className={`w-80 h-56 bg-[#3a2818] rounded-3xl border-b-8 border-r-8 border-[#26180d] relative flex flex-col items-center justify-center overflow-hidden transition-transform duration-300 ${isSparkling ? 'scale-105' : 'scale-100'}`}>
              {/* Inside of suitcase (darker) representing depth */}
              ...
              {/* Straps hanging on the side to look nice */}
