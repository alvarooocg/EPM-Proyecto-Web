...
const PasswordScreen: React.FC<PasswordScreenProps> = ({ onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#010113] overflow-hidden">
      ...
      >
        <div className="bg-[#05051e]/80 p-8 md:p-12 rounded-[23px] flex flex-col items-center">
          ...
          <form onSubmit={handleSubmit} className="w-full space-y-6">
            <div className="relative group">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <Lock className={`w-5 h-5 transition-colors duration-300 ${error ? 'text-red-400' : 'text-zinc-500 group-focus-within:text-indigo-400'}`} />
              </div>
              <input
                type="password"
                value={password}
              ...
              <button
                ...
                className="absolute right-2 top-2 bottom-2 px-4 bg-indigo-600 hover:bg-indigo-500 disabled:bg-zinc-800 disabled:text-zinc-600 text-white rounded-xl transition-all flex items-center justify-center shadow-lg active:scale-95"
              >
                {loading ? (
          ...
          <div className="mt-12 flex items-center gap-2 text-zinc-600 text-xs uppercase tracking-widest font-semibold">
